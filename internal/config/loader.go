...
func Load() (*Config, error) {
	...
	_ = v.BindEnv("strava.redirect_uri", "QUANTLETE_STRAVA_REDIRECT_URI")
	_ = v.BindEnv("strava.webhook_verify_token", "QUANTLETE_STRAVA_WEBHOOK_VERIFY_TOKEN")
	_ = v.BindEnv("storage.data_dir", "QUANTLETE_STORAGE_DATA_DIR")
	_ = v.BindEnv("log.level", "QUANTLETE_LOG_LEVEL")

	// Config file (optional)
	v.SetConfigName("quantlete")
	v.SetConfigType("yaml")
	v.AddConfigPath(".")
	v.AddConfigPath("./config")
	v.AddConfigPath("$HOME/.config/quantlete")
	v.AddConfigPath("/etc/quantlete")
